...
CProgressWnd::CProgressWnd(CWnd* pParent, LPCTSTR pszTitle, BOOL bSnake /* = FALSE */)
{
    CommonConstruct();
    m_strTitle = pszTitle;

    Create(pParent, pszTitle, bSnake);
}
...
BOOL CProgressWnd::Create(CWnd* pParent, LPCTSTR pszTitle, BOOL bSnake /* = FALSE */)
{
    BOOL bSuccess;
    ...
    if (!bSuccess) 
		return FALSE;
	m_wndProgress.SetSnakeStyle(bSnake);
...
BOOL CProgressWnd::GoModal(LPCTSTR pszTitle /*=_T("Progress")"*/, BOOL bSnake /*=FALSE*/)
{
    CWnd *pMainWnd = AfxGetMainWnd();

    if (!::IsWindow(m_hWnd) && !Create(pMainWnd, pszTitle, bSnake))
        return FALSE;
...
int CProgressWnd::StepIt(BOOL bEnsureVisible)
{
    if (!::IsWindow(GetSafeHwnd())) 
        return -1;
...
int CProgressWnd::SetPos(int nPos, BOOL bEnsureVisible)                    
{
    if (!::IsWindow(GetSafeHwnd())) 
        return -1;
...
void CProgressWnd::ResetProgress()
{
    if (!::IsWindow(GetSafeHwnd())) 
        return;

	m_wndProgress.ResetProgress();
}


void CProgressWnd::SetText(LPCTSTR strInfo)
{
    if (!::IsWindow(GetSafeHwnd())) 
        return;

    m_Text.SetWindowText(strInfo);
}
...
{
    if (m_bModal && m_hWnd && ::GetFocus() != m_hWnd)
        SetFocus();
