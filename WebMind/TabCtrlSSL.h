...
public:
	void SetTabColor(int itemID, COLORREF crTab);
// Construction
	CTabCtrlSSL ();
// Destruction
	virtual ~CTabCtrlSSL (void);
// Page Functions
	int		AddSSLPage (LPCTSTR pszTitle, int nPageID, CTabPageSSL* pTabPage, int nImage = -1);
	int		AddSSLPage (LPCTSTR pszTitle, int nPageID,	LPCTSTR pszTemplateName, int nImage = -1);
	int		AddSSLPage (LPCTSTR pszTitle, int nPageID, int nTemplateID, int nImage = -1);
