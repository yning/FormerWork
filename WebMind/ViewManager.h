...
// Implementation
public:
	int GetViewNum(CRuntimeClass* pViewClass);

	CView *GetFirstView(CRuntimeClass *pViewClass);
	CView *GetNextView(CRuntimeClass *pViewClass);

	BOOL GetAddLast();
	void SetAddLast(BOOL last);
...
protected:
	...
	//}}AFX_MSG
    void OnViewManagerToolTip(NMHDR* pTTTStruct, LRESULT * pResult);
	DECLARE_MESSAGE_MAP()
};
