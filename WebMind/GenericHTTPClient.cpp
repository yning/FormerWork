...
//	DWORD dwSize=0;
	...
	if(Connect(szAddress, szAgent, (unsigned short)dwPort))
	{
		...
		{
//			if(!Response((PBYTE)_szHTTPResponseHeader, __SIZE_HTTP_BUFFER, (PBYTE)_szHTTPResponseHTML, __SIZE_HTTP_BUFFER, _dwResponseSize))
			if(!Response())
			{
				bReturn=FALSE;
				_dwResponseSize = 0;
...
BOOL GenericHTTPClient::Response()
{

	BYTE pResponseBuffer[__SIZE_BUFFER]="";	
	DWORD dwNumOfBytesToRead=0;
	DWORD dwHeaderBufferLength = __SIZE_HTTP_BUFFER;
...
//	}
	...
	_dwResponseSize = 0;
	while((dwNumOfBytesToRead=ResponseOfBytes(pResponseBuffer, __SIZE_BUFFER))!=NULL)
	{
		if(dwNumOfBytesToRead > 0)
		{

			if(_szHTTPResponseHTML == NULL)
				_szHTTPResponseHTML = (TCHAR *) ::LocalAlloc(LPTR,  dwNumOfBytesToRead); 
			else
				_szHTTPResponseHTML = (TCHAR *) ::LocalReAlloc(_szHTTPResponseHTML, 
                _dwResponseSize + dwNumOfBytesToRead, LMEM_MOVEABLE | LMEM_ZEROINIT); 

			::CopyMemory( (_szHTTPResponseHTML + _dwResponseSize), pResponseBuffer, dwNumOfBytesToRead);

			_dwResponseSize += dwNumOfBytesToRead;
		}
	}


	::ZeroMemory(_szHTTPResponseHeader, dwHeaderBufferLength);
	if(!::HttpQueryInfo(_hHTTPRequest, HTTP_QUERY_RAW_HEADERS_CRLF, _szHTTPResponseHeader, &dwHeaderBufferLength, NULL))
	{
		_dwError=::GetLastError();
		return FALSE;
	}

	return (_dwResponseSize? TRUE: FALSE);
}
...
DWORD GenericHTTPClient::QueryResponseSize()
{
	return _dwResponseSize;
}

TCHAR *GenericHTTPClient::QueryResponse()
{
	return _szHTTPResponseHTML;
}