...
//////////////////////////////////////////////////////////////////////
#include "StdAfx.h"//added by kosko
#include "HighTime.h"
...
const CHighTime& CHighTime::operator=(const CTime& dateSrc) //这个本来在if defined(USE_MFC) #endif里面，我现在提取出来了
{
    _HighTimeFormat  SrcTime;
    SrcTime.nYear   = dateSrc.GetYear();
    SrcTime.nMonth  = dateSrc.GetMonth();
...
int CHighTime::FindStr(LPCTSTR pszSource, LPCTSTR pszSub, int nStart)
{
	...
	// find first
	LPSTR pszSearch = strstr(pszSource + nStart, pszSub);
...
// formatting
#if defined(USE_MFC)//modified by kosko
...
//the following is added by kosko
#include "Parseit.h"
// Parse the string into the time structure
// TheDate has the following format:
// 	01/01/2004 12:12:23.500
//  mm/dd/yyyy h:m:s.s
BOOL CHighTime::ParseDateTime(LPCSTR TheDate)
{
	CParseIt ParseIt(TheDate, "/ :.");      
	ParseIt.Parse();
	...
	if( N > 0)
		SrcTime.nMonth = (int)ParseIt.GetField(1);
	
	if( N > 1)
		SrcTime.nDay = (int)ParseIt.GetField(2);
	
	if( N > 2)
		SrcTime.nYear = (int)ParseIt.GetField(3);
	
	if( N > 3)
		SrcTime.nHour = (int)ParseIt.GetField(4);
	
	if( N > 4)
		SrcTime.nMinute = (int)ParseIt.GetField(5);
	
	if( N > 5)
		SrcTime.nSecond = (int)ParseIt.GetField(6);
	
	if( N > 6)
		SrcTime.nMilli = (int)ParseIt.GetField(7);
	
	if( N > 7)
		SrcTime.nMicro = (int)ParseIt.GetField(8);
	
	if( N > 8)
		SrcTime.nNano = (int)ParseIt.GetField(9);
