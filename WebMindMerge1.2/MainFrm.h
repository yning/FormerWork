...
#define  IS_FAVICON(nIcon) (nIcon>3)//这么做，是因为我人为的改变了原来的那个位图结构，现在固定位图只有四个，因此只需大于3，便可认为是网站图标
...
extern 	CDictionary *pDict; //词典类
extern	CSegment seg;//分词类
//########################################################
...
public:
	...
	void	InitToolBarImage(BOOL bReplaceIcon=FALSE);//感觉应该是这个或者下面的InitLinkBarImage()
	void	InitMainAnimIcon();
	void	InitLinkBarImage();
	void	InitFavBarImage();//这个应该是收藏栏的image吧
	void	InitSystemBarImage();//系统栏的image?
	void	InitMenuBarImage();//菜单栏的image
	void	InitGoImage();//难道是那个地址栏的GO图片?
	void	InitTaskBarImage();//任务栏图片
...
public:
	...
	int  m_nRTabID;
	int  m_nSwitch;
	int m_tabsAtSameTime;//同时浏览的最大网页数
