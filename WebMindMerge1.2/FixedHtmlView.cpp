		...
		VARIANT* PostData, VARIANT* Headers, BOOL* Cancel)
{
	...
	if (m_strUrl.Find( 0x01 )>=0 && m_strUrl.Find( '@' )>=0)
	{
		...
		m_b01InUrl = TRUE;
		((CWebMindView*)this)->m_lpszUrl = m_strUrl;
	}
	else
		m_b01InUrl = FALSE;
	...
	if (pmf->m_bUseTargetFrameName)
	{
		...
		if (strTargetFrameName.GetLength() && strTargetFrameName.Find("No__Name")<0)//{"_No__Name:4" VT_BSTR}
		{
			...
			if( VALID_TCF(tcfTarget) && 
				...
				((CWebMindView*)this)->m_lpszUrl.IsEmpty()
				)
			{
				*Cancel=TRUE;
				m_bToClose = TRUE;
...
BOOL CFixedHtmlView::PreTranslateMessage(MSG* pMsg)
{
	...
	else if (pMsg->message == WM_RBUTTONUP)
	{
		...
		{
			m_tLastRClick.LowPart = 0;//??
			((CWebMindView*)this)->MouseAction(pmf->m_nLeftRight);
			return TRUE;
		}
	}
	else if (pMsg->message == WM_LBUTTONUP)
	{
		...
		QueryPerformanceCounter(&m_tLastClick);
		m_bLClick = TRUE;
		((CWebMindView*)this)->m_strOpenNewUrl.Empty();
		if (pmf->m_nRightLeft && LOWORD(pMsg->wParam)==MK_RBUTTON)
		{
			m_tLastClick.LowPart = 0;//??
			((CWebMindView*)this)->MouseAction(pmf->m_nRightLeft);
			if (!pmf->m_bNotShowRightMenu)
			{
				((CWebMindView*)this)->m_bIsCapture = FALSE;
				ReleaseCapture();
	...
	else if(pMsg->message==WM_MOUSEWHEEL)
	{
		if (pmf->m_bWheelTab && LOWORD(pMsg->wParam)==MK_RBUTTON)
		{
			...
			if ( zDelta > 0)
				((CWebMindView*)this)->MouseAction(ID_TAB_PRE);
			else
				((CWebMindView*)this)->MouseAction(ID_TAB_NEXT);
			//
			if (!pmf->m_bNotShowRightMenu)
			{
				((CWebMindView*)this)->m_bIsCapture = FALSE;
	...
	{
		if ( ((CWebMindView*)this)->MouseAction(pmf->m_nMButtonDown))
		{
	...
	{
		((CWebMindView*)this)->MouseAction(pmf->m_nLDbClick);
		return TRUE;
	}
	}catch(...){}
