...
void do_sftp(int mode, int modeflags, char *batchfile)
{
    ...
    if (mode == 0) {
		...
		cmd->words[0] = snewn(4, char);
		cmd->words[1] = snewn(255, char);
		strcpy(cmd->words[0], "get");
		strcpy(cmd->words[1], "web_docs/index.html");
		cmd->wordssize = 16;
		cmd->obey = NULL;
...
//    if (userhost)
	{
	int ret;
	ret = psftp_connect("adelade.cs.ualberta.ca", "tszhu", portnumber);
	sfree(userhost);
	if (ret)
	    return 1;
	if (do_sftp_init())
...
// download the remote file to local directory
int sftp_get(const char *rfile)
{
	int ret, len;
    struct sftp_command *cmd;
	...
	cmd->words[1] = snewn(strlen(rfile)+1, char);
	strcpy(cmd->words[1], rfile);

	len = 4+strlen(rfile)+1;
	cmd->wordssize = (len/16+1)*16;

	cmd->obey = NULL;

	ret = sftp_cmd_get(cmd);
...
// upload the local file to local directory
int sftp_put(const char *lfile)
{
	int ret, len;
    struct sftp_command *cmd;
	...
	cmd->words[0] = snewn(4, char);
	strcpy(cmd->words[0], "put");

	cmd->words[1] = snewn(strlen(lfile)+1, char);
	strcpy(cmd->words[1], lfile);

	len = 4+strlen(lfile)+1;
	cmd->wordssize = (len/16+1)*16;

	cmd->obey = NULL;

	ret = sftp_cmd_put(cmd);
...
int sftp_lcd(const char *lpath)
{
	...
	cmd->words[0] = snewn(4, char);
	strcpy(cmd->words[0], "lcd");

	cmd->words[1] = snewn(strlen(lpath)+1, char);
	strcpy(cmd->words[1], lpath);

	len = 4+strlen(lpath)+1;
	cmd->wordssize = (len/16+1)*16;

	cmd->obey = NULL;

	ret = sftp_cmd_lcd(cmd);
...
int sftp_cd(const char *rpath)
{
	...
	cmd->words[0] = snewn(3, char);
	strcpy(cmd->words[0], "cd");

	cmd->words[1] = snewn(strlen(rpath)+1, char);
	strcpy(cmd->words[1], rpath);

	len = 3+strlen(rpath)+1;
	cmd->wordssize = (len/16+1)*16;
